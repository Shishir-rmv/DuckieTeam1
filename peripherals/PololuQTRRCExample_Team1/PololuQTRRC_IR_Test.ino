...
PololuQTRSensorsRC qtrrc((unsigned char[]) {7},
...
{
  Serial.begin(9600);
  Serial.println("went here lol");
  delay(500);
  int i;
  pinMode(13, OUTPUT);
  Serial.println("Calibrating...");    // turn on LED to indicate we are in calibration mode
  for (i = 0; i < 400; i++)  // make the calibration take about 10 seconds
  {
    qtrrc.calibrate();       // reads all sensors 10 times at 2500 us per read (i.e. ~25 ms per call)
  }
  Serial.println("Calibrated");     // turn off LED to indicate we are through with calibration
